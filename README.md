# Tables d'Addition (PWA)

Appli web pour s'entraîner aux tables d'addition. On choisit les tables à
travailler, l'appli pose les additions dans le désordre, et à la fin elle donne
un **rapport d'erreurs** et un **bilan par table**.
...
**Lien** : https://noelim111318.github.io/tables-addition/

Construite sur `pwa-engine` (dossier `engine/` :
service worker, bandeau d'installation, stockage, sons, série de jours). Même
esprit que l'appli des tables de multiplication.

## Fonctionnalités

- **Choix des tables** : coche les tables de 1 à 10 (« Tout sélectionner » /
  « Tout décocher »). Une « table de 7 » = `7 + 1` … `7 + 10`. Une partie =
  chaque `table + 1…10` une fois, mélangées.
- **Pavé numérique intégré** : la réponse s'affiche dans un cadre (pas de champ
  ...
  Les sommes vont jusqu'à 20 : 2 chiffres maximum.
...
- **Les erreurs reviennent** : une addition ratée est **replacée quelques
...
- **🕳️ Calcul à trous** (option) : chaque question cache au hasard le résultat, le premier ou le
  deuxième nombre (`? + 7 = 12`), et il faut trouver le nombre manquant. Après une erreur, la
...
- **Écran de bilan** :
  ...
  - **📋 additions à revoir** : triées par nombre d'erreurs, avec le temps de
    réponse, le **total cumulé d'erreurs** (« total : N », gardé d'une partie à
    l'autre) et un badge **🐢 hésitation** pour les bonnes réponses trop lentes
    (> 4 s) ;
  - **🔁 Réviser mes erreurs** : relance une partie avec **seulement** les
    additions ratées ou hésitantes ;
...
1. **Accueil** — coche les tables, règle l'option son, puis **🚀 C'est parti !**
   (le bouton est grisé tant qu'aucune table n'est cochée).
...
3. **Bilan** — résultats, graphiques, rapport d'erreurs. De là :
   **Réviser mes erreurs**, **Recommencer** (rejoue les mêmes additions) ou
...
cochées par défaut, seuil d'hésitation (`slowMs`, 4 s), nombre de chiffres, mascottes,
phrases du bilan. Aucun réglage n'exige de toucher à `app.js`.

## Ce que l'appli garde en mémoire (dans le navigateur, jamais envoyé ailleurs)

Clés `localStorage`, préfixées par `tables-addition:` :

| Donnée | Clé |
|---|---|
| Tables cochées + options « petits sons », « contre la montre » et « calcul à trous » | `prefs` |
| Total cumulé d'erreurs par addition (`a+b`) | `errors` |
...
Dépôt : https://github.com/Noelim111318/tables-addition
...
3. Attends une minute : le site est publié sur
   `https://noelim111318.github.io/tables-addition/`.
...
```bash
./tools/sync-engine.sh <chemin>/tables-addition
```

puis `./tools/bump-version.sh vX.Y.Z` ici (le cache du service worker inclut
`engine/*`). API du moteur : [`engine/README.md`](engine/README.md).

## Icônes

L'icône (un gros **+** sur une tuile jaune, posée sur le ciel violet étoilé) est
générée par [`tools/make-icons.py`](tools/make-icons.py) :
...
Les couleurs et proportions sont en haut du script. Il écrit `icons/*.png` et
`favicon.ico`.
